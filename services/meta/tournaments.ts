...
export const reportMatchResult = (tourney: Tournament, matchId: string, winnerId: string): Tournament => ({
  ...tourney,
  matches: tourney.matches.map(match =>
...
export const markTournamentReady = (state: TournamentQueueState, participants: TournamentParticipant[]): TournamentQueueState => ({
  ...state,
  status: 'ready',
  readyAt: Date.now(),
  participants
