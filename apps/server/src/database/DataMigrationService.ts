...
export class DataMigrationService {
  ...
  async runPendingMigrations(): Promise<void> {
    ...
    const executedMigrations = new Set(
      status.migrations.filter(m => m.status === 'executed').map(m => m.id)
    );
  ...
  initializeDefaultMigrations(): void {
    ...
    this.registerMigration({
      ...
      down: async () => {
        ...
        // Remove sample data
        await db.query(
          "DELETE FROM user_stats WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'player%'"
        );
    ...
    this.registerMigration({
      ...
      down: async () => {
        ...
        await db.query(
          "DELETE FROM analytics_events WHERE event_type IN ('user_login', 'game_start', 'game_end', 'user_logout')"
        );
      },
    });
    ...
    this.registerMigration({
      ...
      up: async () => {
        ...
        await db.query(
          'CREATE INDEX IF NOT EXISTS idx_analytics_events_user_timestamp ON analytics_events(user_id, server_timestamp DESC)'
        );
        await db.query(
          'CREATE INDEX IF NOT EXISTS idx_user_stats_score_level ON user_stats(total_score DESC, level DESC)'
        );
  ...
  async validateMigrationIntegrity(): Promise<{
    ...
    try {
      ...
      const executedIds = new Set(
        migrationStatus.migrations.filter(m => m.status === 'executed').map(m => m.id)
      );
