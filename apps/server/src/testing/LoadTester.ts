...
export class LoadTester {
  ...
  private async executeRequest(
    ...
    try {
      ...
      result.averageResponseTime =
        (result.averageResponseTime * (result.totalRequests - 1) + responseTime) /
        result.totalRequests;
  ...
  private calculateFinalMetrics(result: LoadTestResult): void {
    result.errorRate =
      result.totalRequests > 0 ? (result.failedRequests / result.totalRequests) * 100 : 0;
    ...
    if (result.performanceMetrics.cpu.length > 0) {
      const avgCpu =
        result.performanceMetrics.cpu.reduce((sum, val) => sum + val, 0) /
        result.performanceMetrics.cpu.length;
      const avgMemory =
        result.performanceMetrics.memory.reduce((sum, val) => sum + val, 0) /
        result.performanceMetrics.memory.length;
      const avgNetwork =
        result.performanceMetrics.network.reduce((sum, val) => sum + val, 0) /
        result.performanceMetrics.network.length;
