...
export class AuthService {
  ...
  static async createDefaultAdmin() {
    ...
    if (!adminPassword) {
      if (isDev) {
        logger.warn(
          '⚠️ SECURITY: Using default admin password in development. Set ADMIN_PASSWORD env var for production!',
          {}
        );
    ...
    if (!users.has(adminId)) {
      ...
      logger.info('Default admin user created (Argon2id Secured)');
    }
  }
