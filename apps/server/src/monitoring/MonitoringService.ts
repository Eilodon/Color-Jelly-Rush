...
export class MonitoringService {
  ...
  private checkAlerts(metrics: ServerMetrics): void {
    ...
    if (metrics.memory.percentage > this.THRESHOLDS.memoryUsage) {
      this.createAlert(
        'warning',
        `High memory usage: ${(metrics.memory.percentage * 100).toFixed(1)}%`
      );
    }
    ...
    if (metrics.game.totalPlayers > this.THRESHOLDS.maxPlayers) {
      this.createAlert(
        'warning',
        `High player count: ${metrics.game.totalPlayers}/${this.THRESHOLDS.maxPlayers}`
      );
    }
  ...
  private createAlert(type: 'warning' | 'critical', message: string): void {
    ...
    const existingAlert = this.alerts.find(
      a => !a.resolved && a.message === message && Date.now() - a.timestamp < 60000 // Within last minute
    );
  ...
  private cleanup(): void {
    ...
    this.alerts = this.alerts.filter(
      a => !a.resolved || (a.resolved && a.resolvedAt && a.resolvedAt > oneHourAgo)
    );
  }
