...
export class PerformanceProfiler {
  ...
  async profile<T>(operation: string, fn: () => Promise<T> | T, context?: any): Promise<T> {
    if (!this.isProfiling) {
      return fn();
    }
  ...
  getReport(): {
    ...
    const operations = Array.from(this.operationStats.values()).sort(
      (a, b) => b.avgDuration - a.avgDuration
    );

    const bottlenecks = operations
      .filter(op => op.efficiency < 50 || op.avgDuration > 100)
      .slice(0, 10);
  ...
  private generateRecommendations(bottlenecks: PerformanceReport[]): string[] {
    ...
    for (const bottleneck of bottlenecks) {
      ...
      if (bottleneck.memoryImpact > 1000000) {
        // 1MB
        recommendations.push(
  ...
  getMetricsByOperation(operation: string, count: number = 100): PerformanceMetrics[] {
    return this.metrics.filter(m => m.operation === operation).slice(-count);
  }
