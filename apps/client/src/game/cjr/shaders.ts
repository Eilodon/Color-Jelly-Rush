...
// EIDOLON-V: Type definitions for shader uniforms (type safety)
export interface JellyShaderUniforms {
  uTime: { value: number; type: 'f32' };
  uRadius: { value: number; type: 'f32' };
  uAberration: { value: number; type: 'f32' };
  uJellyColor: { value: number[] | Float32Array; type: 'vec3<f32>' };
  uAlpha: { value: number; type: 'f32' };
  uEnergy: { value: number; type: 'f32' };
  uSquish: { value: number; type: 'f32' };
  uWobble: { value: number; type: 'f32' };
  uPulsePhase: { value: number; type: 'f32' };
  uDeformMode: { value: number; type: 'i32' };
  uPatternMode: { value: number; type: 'i32' };
  translationMatrix: { value: number[] | Float32Array; type: 'mat3x3<f32>' };
  projectionMatrix: { value: number[] | Float32Array; type: 'mat3x3<f32>' };
}

export interface JellyShaderResources {
  uniforms: {
    uniforms: JellyShaderUniforms;
  };
}
