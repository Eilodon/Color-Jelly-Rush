...
export class CollisionSystem {
  ...
  static pointCircleCollision(point: Vector2, circlePos: Vector2, radius: number): boolean {
  ...
  static pointRectCollision(point: Vector2, rectPos: Vector2, rectSize: Vector2): boolean {
    return (
      point.x >= rectPos.x &&
  ...
  static lineCircleCollision(
    ...
    const t = FastMath.clamp((toCircle.x * lineVec.x + toCircle.y * lineVec.y) / lineLenSq, 0, 1);
...
export class SpatialOptimizer {
  ...
  static circlesIntersect(pos1: Vector2, radius1: number, pos2: Vector2, radius2: number): boolean {
    return CollisionSystem.circleCollision(pos1, radius1, pos2, radius2);
  }

  // EIDOLON-V FIX: Fast point in circle test
  static pointInCircle(point: Vector2, circlePos: Vector2, radius: number): boolean {
    return CollisionSystem.pointCircleCollision(point, circlePos, radius);
  }

  // EIDOLON-V FIX: Fast distance comparison
  static isWithinDistance(pos1: Vector2, pos2: Vector2, maxDistance: number): boolean {
  ...
  static closestPointOnCircle(point: Vector2, circlePos: Vector2, radius: number): Vector2 {
    const dx = point.x - circlePos.x;
    const dy = point.y - circlePos.y;
