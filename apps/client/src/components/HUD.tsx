...
const HUD: React.FC<HUDProps> = memo(({ gameStateRef }) => {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none select-none font-ui overflow-hidden">
      {/* Match Meter */}
      <div
        ref={winHoldContainerRef}
        className={`absolute top-6 left-1/2 -translate-x-1/2 flex flex-col items-center transition-transform duration-300`}
      >
        <div className="relative w-36 h-36">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span ref={percentRef} className="text-3xl font-black font-display text-white">
              0%
            </span>
        ...
        <div
          ref={colorHintRef}
          className={`mt-2 text-sm font-bold px-4 py-1 rounded-full bg-black/40 border border-white/10 text-white transition-opacity opacity-0`}
        >
          ...
        </div>
      </div>
      ...
      <div className="absolute top-6 right-6 text-right">
        <div className="text-white font-display text-xl drop-shadow-lg">
          SCORE{' '}
          <span ref={scoreRef} className="text-yellow-400">
            0
          </span>
        </div>
        <div ref={waveRef} className="text-white/60 text-xs font-mono uppercase">
          WAVE READY
        </div>
      </div>
    </div>
