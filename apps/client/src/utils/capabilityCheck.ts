...
export function checkEnvironmentCapabilities(): boolean {
    ...
    if (isSABSupported) {
        console.info('%c✅ SharedArrayBuffer is defined.', 'color: green');
    } else {
    ...
    if (isCrossOriginIsolated) {
        console.info('%c✅ Cross-Origin Isolated: TRUE', 'color: green');
    } else {
    ...
    if (supportsWorker) {
        console.info('%c✅ Web Workers: SUPPORTED', 'color: green');
    } else {
    ...
    if (ready) {
        console.info('%c🚀 MULTITHREADING READY', 'color: #00ff00; font-weight: bold');
    } else {
