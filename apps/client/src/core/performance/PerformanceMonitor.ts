...
export class PerformanceMonitor {
  ...
  enableDebugMode(enabled: boolean) {
    ...
    if (enabled) {
      clientLogger.debug(
        '[PerformanceMonitor] Debug mode ENABLED - tracking GC pauses and frame outliers'
      );
    }
  }
  ...
  updateFrame() {
    ...
    if (this.debugMode && deltaTime > this.FRAME_OUTLIER_THRESHOLD_MS) {
      ...
      console.warn(
        `[GC Debug] Frame outlier: ${deltaTime.toFixed(2)}ms (threshold: ${this.FRAME_OUTLIER_THRESHOLD_MS}ms)`
      );
    }
    ...
    if (this.windowFrameCount >= 10) {
      ...
      this.metrics.fps =
        windowMs > 0 ? Math.round((this.windowFrameCount * 1000) / windowMs) : this.metrics.fps;
    ...
    if ('memory' in performance) {
      ...
      if (this.debugMode && this.lastMemorySnapshot > 0) {
        ...
        if (memoryDrop >= this.GC_MEMORY_SPIKE_MB) {
          ...
          clientLogger.warn(
            `[GC Debug] Estimated GC pause #${this.estimatedGCPauses}: memory dropped ${memoryDrop}MB (${this.lastMemorySnapshot}MB -> ${currentMemoryMB}MB)`
          );
        }
      }
  ...
  getGCStats(): GCStats {
    ...
    const avgOutlierMs =
      outlierCount > 0 ? this.frameTimeOutliers.reduce((a, b) => a + b, 0) / outlierCount : 0;
    return {
      outlierCount,
  ...
  private checkPerformanceThresholds() {
    ...
    if (avgFPS < this.thresholds.minFPS) {
      ...
      memoryUsage < this.thresholds.maxMemoryMB * 0.7
    ) {
      this.upgradeQuality();
    }
  }
