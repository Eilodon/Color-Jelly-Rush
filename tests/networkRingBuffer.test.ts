...
describe('NetworkClient - Ring Buffer Optimization', () => {
    ...
    describe('Map Reuse (Zero Allocation)', () => {
        test('should reuse same Map objects across writes', () => {
            ...
            clientAny.pushSnapshot({
                players: new Map([['p1', { position: { x: 100, y: 200 }, velocity: { x: 0, y: 0 }, radius: 50 }]]),
                bots: new Map(),
                food: new Map()
            });
        ...
        test('should clear Maps before repopulating', () => {
            ...
            // First write
            clientAny.pushSnapshot({
                players: new Map([['p1', { position: { x: 1, y: 1 }, velocity: { x: 0, y: 0 }, radius: 10 }]]),
    ...
    describe('Interpolation with Ring Buffer', () => {
        test('should find correct snapshot pair for interpolation', () => {
            ...
            clientAny.snapshotBuffer[0].players = new Map([['p1', { x: 0, y: 0, vx: 0, vy: 0, radius: 10 }]]);

            clientAny.snapshotBuffer[1].time = now - 200;
            clientAny.snapshotBuffer[1].players = new Map([['p1', { x: 50, y: 50, vx: 0, vy: 0, radius: 10 }]]);

            clientAny.snapshotBuffer[2].time = now - 100;
            clientAny.snapshotBuffer[2].players = new Map([['p1', { x: 100, y: 100, vx: 0, vy: 0, radius: 10 }]]);
        ...
        test('should use fallback when no suitable snapshot found', () => {
            ...
            mockGameState.players.push(mockPlayer);
            clientAny.snapshotBuffer[0].players = new Map([['p1', { x: 999, y: 999, vx: 0, vy: 0, radius: 10 }]]);
