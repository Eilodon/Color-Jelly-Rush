...
export class RateLimiter {
  ...
  async init(redis: RedisManager): Promise<void> {
    try {
      ...
      logger.warn('RateLimiter: Redis unavailable, falling back to in-memory (NOT recommended for production)');
  ...
  private async checkLimitRedis(identifier: string): Promise<RateLimitResult> {
    ...
    try {
      ...
      logger.error('Redis rate limit check failed, allowing request', { identifier }, error as Error);
