...
export class ServerValidator {
  ...
  static validatePosition(
    ...
    if (distance > maxDistance) {
      return {
        ...
        correctedPosition: this.interpolatePosition(oldPosition, newPosition, maxDistance / distance)
      };
    }
  ...
  static validateCollision(
    ...
    const distance = Math.sqrt(
      Math.pow(attacker.position.x - targetX, 2) +
      Math.pow(attacker.position.y - targetY, 2)
    );
