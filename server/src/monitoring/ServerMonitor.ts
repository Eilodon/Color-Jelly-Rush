...
export class ServerMonitor {
   ...
   */
  private getGameMetrics(): { activeRooms: number; totalPlayers: number; averageLatency: number; updateRate: number } {
  ...
  private checkThresholds(metrics: ServerMetrics): void {
    ...
    if (metrics.cpu > this.thresholds.cpu.critical) {
      this.addAlert('cpu', 'critical', `CPU usage critically high: ${metrics.cpu.toFixed(1)}%`, metrics.cpu, this.thresholds.cpu.critical);
    } else if (metrics.cpu > this.thresholds.cpu.warning) {
      this.addAlert('cpu', 'medium', `CPU usage high: ${metrics.cpu.toFixed(1)}%`, metrics.cpu, this.thresholds.cpu.warning);
    }

    // Memory alerts
    if (metrics.memory.percentage > this.thresholds.memory.critical) {
      this.addAlert('memory', 'critical', `Memory usage critically high: ${metrics.memory.percentage.toFixed(1)}%`, metrics.memory.percentage, this.thresholds.memory.critical);
    } else if (metrics.memory.percentage > this.thresholds.memory.warning) {
      this.addAlert('memory', 'medium', `Memory usage high: ${metrics.memory.percentage.toFixed(1)}%`, metrics.memory.percentage, this.thresholds.memory.warning);
    }

    // Latency alerts
    if (metrics.game.averageLatency > this.thresholds.latency.critical) {
      this.addAlert('game', 'critical', `Average latency critically high: ${metrics.game.averageLatency.toFixed(0)}ms`, metrics.game.averageLatency, this.thresholds.latency.critical);
    } else if (metrics.game.averageLatency > this.thresholds.latency.warning) {
      this.addAlert('game', 'medium', `Average latency high: ${metrics.game.averageLatency.toFixed(0)}ms`, metrics.game.averageLatency, this.thresholds.latency.warning);
    }

    // Update rate alerts
    if (metrics.game.updateRate < this.thresholds.updateRate.critical) {
      this.addAlert('game', 'critical', `Update rate critically low: ${metrics.game.updateRate.toFixed(1)} FPS`, metrics.game.updateRate, this.thresholds.updateRate.critical);
    } else if (metrics.game.updateRate < this.thresholds.updateRate.warning) {
      this.addAlert('game', 'medium', `Update rate low: ${metrics.game.updateRate.toFixed(1)} FPS`, metrics.game.updateRate, this.thresholds.updateRate.warning);
    }

    // Network alerts
    if (metrics.network.messagesPerSecond > this.thresholds.messagesPerSecond.critical) {
      this.addAlert('network', 'critical', `Message rate critically high: ${metrics.network.messagesPerSecond.toFixed(0)}/s`, metrics.network.messagesPerSecond, this.thresholds.messagesPerSecond.critical);
    } else if (metrics.network.messagesPerSecond > this.thresholds.messagesPerSecond.warning) {
      this.addAlert('network', 'medium', `Message rate high: ${metrics.network.messagesPerSecond.toFixed(0)}/s`, metrics.network.messagesPerSecond, this.thresholds.messagesPerSecond.warning);
    }
  }
   ...
   */
  private addAlert(type: PerformanceAlert['type'], severity: PerformanceAlert['severity'], message: string, value: number, threshold: number): void {
  ...
  getPerformanceSummary(): {
    ...
    const warningAlerts = recentAlerts.filter(a => a.severity === 'medium' || a.severity === 'high').length;

    let status: 'healthy' | 'warning' | 'critical' = 'healthy';
    if (criticalAlerts > 0) {
      status = 'critical';
    } else if (warningAlerts > 0 || cpuValues[cpuValues.length - 1] > 70 || memoryValues[memoryValues.length - 1] > 80) {
      status = 'warning';
    }
